      ...
      if (_.has(command, 'error') && (!_.isNull(command['error']) && !_.isEmpty(command['error']))) {
        ...
        switch (self.pending[command['id']].method) {
          ...
          case 'mining.authorize':
            self.authorized = command['error'] == null; //tweak for p2pool
            break;
        }
        ...
        delete self.pending[command['id']];
          ...
          self.$class.debug('Mismatched call id, was expecting ' + self.currentId + ' but got ' + command['id'] + " - assuming broadcast", command, self.pending);
